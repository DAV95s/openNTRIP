...
import org.dav95s.openNTRIP.crs.gridShift.GridNode;
import org.dav95s.openNTRIP.protocols.rtcm.assets.CRS2;
...
public class MSG1023 implements CRS2 {
    ...
    public MSG1023(byte[] msg) {
        ...
        for (int i = 0; i < gridMap.length; i++) {
            ...
            gridMap[i].dNorth = Normalize.normalize(bitUtils.getSignedInt(9) * 0.00003d, 8);
            gridMap[i].dEast = Normalize.normalize(bitUtils.getSignedInt(9) * 0.00003d, 8);
            gridMap[i].dH = Normalize.normalize(bitUtils.getSignedInt(9) * 0.00003d, 8);
        }
    ...
    public void setLat0(double lat0) {
        double normalized = Normalize.normalize(lat0, 6);
    ...
    public void setLon0(double lon0) {
        double normalized = Normalize.normalize(lon0, 6);
    ...
    public void setdLat0(double dLat0) {
        double normalized = Normalize.normalize(dLat0, 6);
    ...
    public void setdLon0(double dLon0) {
        double normalized = Normalize.normalize(dLon0, 6);
        checkArgument(0 <= normalized && normalized <= 0.56875);
    ...
    public void setMdLat(double mdLat) {
        double normalized = Normalize.normalize(mdLat, 4);
    ...
    public void setMdLon(double mdLon) {
        double normalized = Normalize.normalize(mdLon, 4);
        checkArgument(-0.127 <= normalized && normalized <= 0.127);
        MdLon = normalized;
    }
    ...
    public void setMdH(double mdH) {
        double normalized = Normalize.normalize(mdH, 4);
